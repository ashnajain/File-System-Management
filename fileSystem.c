...
static char* file_to_d[500];
static char* dir_to_file[500];

static char* users[100];
...
int get_curr_dir(char* filename,int inode_fileget_curr);

/*
Function to print the files and directories like a tree structure.
*/
void tree()
{
	...
	for (i = 2; i < curr_dir->filecount + 2; i++) {
		...
		if (node->filecode == DIRECTORY){
			printf("%s\t", curr_dir->filename[i]);	//printing the directories
			}
			}
	printf("\n / \t | \t /\n");
	for (i = 2; i < curr_dir->filecount + 2; i++) {
		read_inode(curr_dir->inode_num[i]);
		if (node->filecode == FILE)			//printing the files
			printf("%s\t", curr_dir->filename[i]);
		}
...
/* initialize function called by shell.
...
/* initializes superblocks and writes its information to fisk.
 */
static void initialize_super_blocks() {
	int* super_buffer;
	int i;
...
/* initializes all inode blocks, setting all to default except root.
 * returns: 0 if successful, -1 otherwise.
 */
static void initialize_inode_blocks() {
	int i, j, k;
...
/* initializes the root directory writing out to its data blocks.
 * returns 0 if successful, -1 otherwise.
 */
static void initialize_root_dir() {	
	int i, j, k;
...
/* reads current superblock information from fisk into superblock struct.
 * returns : 0 if successful, -1 otherwise.
 */
...
/* writes all super block information to fisk.
 * returns: 0 if successful, -1 otherwise.
 */
static void write_super_blocks() {
	int* super_buffer;
...
/* grabs the next free inode from the inode freelist, setting
 *	it to occupied.
 * returns: 0 if successful, -1 otherwise.
 */
static int get_free_inode() {
...
/* reads inode specified by inode_index into the node struct.
 */
static void read_inode(int inode_index) {
	int i;
	int inode_block_num;
...
/* adds a free new inode for a newly created file and reserves
...
/* zeroes out an inode's diskmap and sets the inode specified by
...
/* grabs a free data block from the data block free list and sets it
 *	to occupied.
 * returns: 0 if successful, -1 otherwise.
 */
static int get_free_data_block() {
...
/*
Function to map a file to a directory.
*/

void file_to_dir(char *file,char *dir,int inode_file)
{
	int i=0;
	file_to_d[inode_file] = dir;	//The file at specific inode is mapped to a directory.
	dir_to_file[inode_file] = file;
}

/* creates a new file, setting up its inode and reserving
 *	data blocks for it.
 * returns: 0 if successful, -1 otherwise.
 */
int create_file(int filesize, char* filename,char* oldDir) {
	...
	for (i = 0; i < (curr_dir->filecount); i++) {
	...
	file_to_dir(filename,oldDir,inode_index);
	return 0;
}

/* creates a new directory, setting up its inode and reserving
 *	data blocks for it.
 * returns: 0 if successful, -1 otherwise.
 */
int create_dir(char* dirname) {
...
/* general create function that calls either create_file() or
...
int fs_create(char* name, int flag, int filesize,char* oldDir) {
	...
	if (flag == FILE) {
		result = create_file(filesize, name,oldDir);
		return result;
	}
 ...
 */
int get_curr_dir(char* filename,int inode_file) {
...
/* changes current directory to a directory in current directory's
...
/* Deletes file specified by filename. Nulls out the diskmap of inode
 ...
 * returns: inode of the file if successfully opened. -1 otherwise.
 */
int fs_open(char* filename) {
	int i;
	int file_inode = -1;
...
/*
Function to list the contents of current directory
*/
int effs_ls() {
	int i;
	int file_inode = -1;
	...
	for (i = 0; i < (curr_dir->filecount+2); i++) {
		{
			file_inode = curr_dir->inode_num[i];
			printf("file name:%s node:%d",curr_dir->filename[i],file_inode);
			break;
		}
	}
...
/* Writes content from file_content to the data blocks in the diskmap
...
int fs_write(int inode_index, void *file_content, int filesize) {
	...
	// remaining fs_write() when to stop writing
	remaining = filesize;
...
/* Helper function of fs_read() that transfers
...
/* Reads content from the data blocks in the diskmap of inode_index
...
int fs_read(char* filename, void *file_content, int filesize) {
	...
	// remaining tells fs_read() when to stop reading
	remaining = filesize;
...
/* syncs open file descriptor fildes to disk.
 * returns:0 if succeeded. Should succeed.
 */
int fs_sync(int fildes) {
	syncDisk();
	return 0;
...
/* Syncs file descriptor fildes and closes it (done in shell).
 * returns: 0 if succeeded. Should succeed.
 */
int fs_close(int fildes) {
	fs_sync(fildes);
